...
For nonlinear relationships, non-linear transformations will help the model fit better. To understand if the model would benefit from a non-linear transformation, it helps to plot and visualize the data first. For our data, we believe the percentage of single, detached dwelling units in a given census tract may not be logarithmically relational. However, in order to verify this, we will run a log transformation to compare. If the log-transformed version has statistically-significant variables and if the model fit (as indicated by the R-square value) improves, then we will use this logarithmic-transformed model. The model-fit (R squared value) is how we choose which model to rely on.
...
coeff_names_logged <- c("Constant" = "(Intercept)",
                 ...
                 "Percentage of SSDDU (Logged)" = "log2_pct_sf_home",
...
model_log <- lm(log2_avg_sf_home_val ~ log2_pct_sf_home + log2_pct_moved + maj_white + type, data_log)

export_summs(full_model, centered_model, centered_model_scaled, model_log,
             error_format = "(p = {p.value})",
             error_pos = "same",
             model.names = c("Initial\n(price in dollars)", 
                             "Centered\n(price in dollars)", 
