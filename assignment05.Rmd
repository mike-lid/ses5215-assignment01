...
We scale our data by dividing the average price of SDDU in a census tract by 10,000, because we are interested in grouping unit prices by 10,000. We are less interested to see the difference in prices by dollars, but consider the 10,000 breakdown of unit prices. We chose not to scale the percentage of SDDU's in a census tract, because it is already grouped into percentages and we are interested to see the effect of these by each percentage. We did, however, scale the percentage of residents moved by dividing by 5, so that we could categorize the percentages into 5% increments.
...
For nonlinear relationships, non-linear transformations will help the model fit better. To understand if the model would benefit from a non-linear transformation, it helps to plot and visualize the data first. For our data, we believe the percentage of single, detached dwelling units in a given census tract may not be logarithmically relational. However, in order to verify this, we will run a log transformation to compare. If the log-transformed version has statistically-significant variables and if the model fit (as indicated by the R-square value) improves, then we will use this logarithmic-transformed model.
