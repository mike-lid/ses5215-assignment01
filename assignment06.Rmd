...
# interactions
Our preferred model from above took the log of all the continuous variables in our data set before running a multivariate regression analysis. From this model, we see that the higher percentages of residents who moved in the year are associated with higher average prices of SDDUs in a given census tract. In addition, when the percentage of residents who moved in a census tract is controlled, census tracts that are majority white tend to have higher average prices of SDDUs. Thus, the following describes the interaction modelling we did to see how the model fit changes when various variables are interacted **(ie. when we consider how the presence of one variable may overlap in their )**

## interacting the percentage of SDDUs with racial makeup
When we modeled how the racial makeup (majority white or not) of a census tract affects the relationship between percentage of SDDUs and the log of SDDU home values, the model fit (R2) did not increase.
```{r}
...
When we included interactions among all variables, we did get a model with an R2 value of 0.31. However, at that point the model had become overly complex and the results were becoming less and less meaningful. That model was sacrificing clarity and conciseness for one one-hundredth better fit.
